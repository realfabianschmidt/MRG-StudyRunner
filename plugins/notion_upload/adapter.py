...
Enable:   set "notion": { "enabled": true, ... } in settings/hardware_settings.json
"""
...
def _refresh_config_for_retry(
    ...
    try:
        ...
        from server_app.services.study_config_service import load_config, load_study
        ...
        try:
            ...
            config_file = base_dir / "settings" / "study_config.json"
            studies_dir = base_dir / "saved_studies"
            current_config = load_config(config_file)
            if str(current_config.get("study_id") or "").strip() == study_id:
                return current_config
            return load_study(studies_dir, study_id)
...
def _persist_study_database_id(config_data: dict[str, Any]) -> None:
    try:
        from flask import current_app
        from server_app.services.study_config_service import save_config, save_study
