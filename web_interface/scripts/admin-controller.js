...
function renderAccessInfo() {
  ...
  if (adminTarget) adminTarget.textContent = adminUrl;
  if (participantTarget) participantTarget.textContent = participantUrl;
  if (hint) {
...
function renderAccessInfoError() {
  ...
  const participantTarget = $('access-participant-url');
  const hint = $('access-hint');
  if (adminTarget) adminTarget.textContent = `${window.location.origin}/admin`;
  if (participantTarget) participantTarget.textContent = window.location.origin;
  if (hint) hint.textContent = 'Runtime info is unavailable. The current browser origin is shown as fallback.';
}

async function copyAccessUrl(kind) {
  const value = kind === 'admin'
    ? $('access-admin-url')?.textContent
    : $('access-participant-url')?.textContent;
  if (!value || value === 'Loading ...') {
    showToast('No link available yet', 'error');
    return;
  }
...
function handleListClick(event) {
  ...
  if (removeButton) {
    ...
    if (qType === 'participant-id' || qType === 'finish') {
      showToast('Start- und End-Karten kÃ¶nnen nicht entfernt werden.', 'error');
      return;
    }
...
async function deleteStudy(id) {
  if (!confirm(`MÃ¶chtest du die Studie "${id}" wirklich unwiderruflich lÃ¶schen?`)) return;
  try {
    ...
    showToast('Studie gelÃ¶scht', 'success');
    await loadRecentStudies();
  } catch(e) {
    ...
    listEl.innerHTML = studies.map(s => `
      <div class="hub-recent-item" data-study-id="${escapeHtml(s.id)}" style="justify-content: flex-start; padding: 12px 16px;">
        ...
        <div class="hub-recent-actions" style="display: flex; gap: 6px;">
          ...
          <button class="btn-icon-only" data-action="delete" title="LÃ¶schen" style="color: #D32F2F; border-color: rgba(211,47,47,0.3);"><i class="iconoir-trash"></i></button>
        </div>
      </div>
    `).join('');
...
// â”€â”€ Study Settings â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function saveStudySettings() {
  ...
  $('study-settings-modal').hidden = true;
  markUnsaved();
  showToast('Studien-Settings temporÃ¤r Ã¼bernommen. Bitte Studie speichern!', 'info');
}
