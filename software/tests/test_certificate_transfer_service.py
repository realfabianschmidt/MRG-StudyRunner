...
class CertificateTransferServiceTests(unittest.TestCase):
    def _create_ca(self, settings_dir: Path) -> dict:
        with mock.patch(
            "study_runner.runtime_core.settings.ssl_service._local_certificate_names",
    ...
    def test_failed_second_atomic_write_restores_original_ca_pair(self) -> None:
        with tempfile.TemporaryDirectory() as source, tempfile.TemporaryDirectory() as target:
            ...
            from study_runner.runtime_core.delivery import certificate_transfer_service as service
