"""Extension SDK tests (Phase 5j, docs/architecture-1.0-umbau.md).
...
server uses, not a copy of it (see tools/extension_sdk.py's own docstring).
"""
...
from tools import extension_sdk as sdk  # noqa: E402
...
class SchemaReferenceTests(unittest.TestCase):
    ...
    `python tools/extension_sdk.py schema --write` -- the fix is to run that
