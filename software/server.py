...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--emotion-worker":
        from study_runner.extensions.sensors.camera_emotion.worker.server import main as run_emotion_worker

        raise SystemExit(run_emotion_worker(sys.argv[2:]))
    if len(sys.argv) > 1 and sys.argv[1] == "--emotion-worker-self-test":
        from study_runner.extensions.sensors.camera_emotion.worker.server import self_test_main
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--brainbit-cli":
        ...
        from study_runner.extensions.sensors.brainbit.brainbit_realtime_cli import main as run_brainbit_cli
