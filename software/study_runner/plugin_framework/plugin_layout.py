"""Trusted built-in plugin roots, shared by discovery and driver startup."""
...
TEST_EXTRA_PLUGIN_ROOT_PATH_ENV_VAR = "STUDY_RUNNER_TEST_EXTRA_PLUGIN_ROOT"
TEST_EXTRA_PLUGIN_ROOT_PACKAGE_ENV_VAR = "STUDY_RUNNER_TEST_EXTRA_PLUGIN_PACKAGE"


def trusted_roots() -> tuple[tuple[Path, str], ...]:
    package = Path(__file__).resolve().parent.parent
    roots = [
        (package / "plugins" / category, f"study_runner.plugins.{category}")
        for category in CATEGORIES
    ]
    extra_path = os.environ.get(TEST_EXTRA_PLUGIN_ROOT_PATH_ENV_VAR, "").strip()
    extra_package = os.environ.get(TEST_EXTRA_PLUGIN_ROOT_PACKAGE_ENV_VAR, "").strip()
...
def resolve_plugin(plugin_key: str) -> tuple[Path, str]:
    matches = []
