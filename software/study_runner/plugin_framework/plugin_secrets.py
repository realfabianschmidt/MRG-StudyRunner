...
*credentials* for them: `study_settings` is serialized verbatim into the
exported `.study-runner` file, so an API key placed there would travel to
whoever receives the study.
...
def _credential_declarations() -> dict[str, dict[str, Any]]:
    ...
    """
    from .registry import get_plugin_manifests
