...
export function renderEditor(q) {
  const words = (q.words ?? defaultQuestion.words).join('\n');
  return `
    <div class="field">
...
export function collectConfig(el) {
  ...
  return {
    ...
    words: words.length ? words : structuredClone(defaultQuestion.words),
...
export function configureCard(defaults) { defaultQuestion = defaults['word-cloud']; }
export const metaByType = { 'word-cloud': meta };
