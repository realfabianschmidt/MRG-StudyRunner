"""Trusted, manifest-driven discovery for built-in integration plugins.

Only plugin folders shipped inside :mod:`study_runner.integrations` are
...
DEFAULT_PACKAGE_NAME = "study_runner.integrations"
...
def discover_plugin_catalog(
    ...
    *,
    ...
    """Discover built-in plugins below one trusted integrations directory.

    ``integrations_dir`` and ``package_name`` are parameters primarily so the
    isolation rules can be tested with a temporary package.  Production uses
    this module's own directory and never accepts either value from a request.
    """

    root = Path(integrations_dir or Path(__file__).resolve().parent).resolve()
