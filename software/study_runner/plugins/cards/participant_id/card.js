...
const FIELD_META = {
  ...
  age_group: { kind: 'select', configurable: true, label: ['cards.participant.ageGroup', 'Age group'] },
  gender: { kind: 'select', configurable: true, label: ['cards.participant.gender', 'Gender'] },
...
function defaultOptions(fieldKey) {
  return [...(defaultQuestion?.fields?.[fieldKey]?.options || [])];
}

function cloneDefaultFields() {
  const fields = JSON.parse(JSON.stringify(defaultQuestion.fields));
...
export function configureCard(defaults) { defaultQuestion = defaults['participant-id']; }
meta.hideSharedPrompt = true;
export const metaByType = { 'participant-id': meta };
