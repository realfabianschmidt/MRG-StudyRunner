...
def _warmup_deepface() -> None:
    ...
    model_path = _deepface_model_path()
    MODEL_STATE["model_asset_path"] = str(model_path)
    if not _asset_is_valid(model_path):
        error = (
            f"{DEEPFACE_EMOTION_MODEL_NAME} is missing or has the wrong SHA-256. "
            "Automatic model downloads are disabled; review THIRD_PARTY_NOTICES.md "
            "and provision the separately licensed model explicitly."
        )
        MODEL_STATE["model_ready"] = False
        MODEL_STATE["model_error"] = error
        MODEL_STATE.update(_classify_model_error(error))
        print(f"[EmotionWorker] DeepFace warmup skipped: {error}", file=sys.stderr)
        return
    try:
...
def _deepface_home() -> Path:
    ...
    if raw_home:
        ...
        storage_root = Path(os.environ["STUDY_RUNNER_DATA_DIR"]).expanduser()
        canonical = storage_root / "runtime" / "camera_emotion" / "worker" / "deepface_home"
        legacy = storage_root / "runtime" / "local_emotion_worker" / "deepface_home"
        deepface_home = legacy if legacy.exists() and not canonical.exists() else canonical
    else:
...
def _asset_is_valid(path: Path) -> bool:
    try:
        return (
            path.is_file()
            and path.stat().st_size >= DEEPFACE_EMOTION_MODEL_MIN_BYTES
