...
DEFAULT_WORKER = {
    "script_path": "study_runner/plugins/sensors/camera_emotion/worker/server.py",
    "log_dir": "study_runner/plugins/sensors/camera_emotion/worker/logs",
    "model_assets_dir": "study_runner/plugins/sensors/camera_emotion/worker/model_assets",
    "deepface_home": "study_runner/plugins/sensors/camera_emotion/worker/deepface_home",
}
...
_SOFTWARE_ROOT = get_project_base_dir()
...
def _local_worker_support_error() -> str | None:
    """Return an actionable reason when this Python cannot host DeepFace.

    TensorFlow no longer publishes a CPython 3.12 wheel for macOS Intel.  The
    rest of Study Runner, including canonical XDF recording, is supported on
    that target, so fail only the local analysis mode and direct operators to
    the manifest-declared remote mode.
    """

    machine = platform.machine().strip().lower()
    if sys.platform == "darwin" and machine in {"amd64", "x86_64"}:
        return (
            "Local camera emotion analysis is unavailable on macOS Intel with "
            "Python 3.12 because TensorFlow provides no compatible wheel. Set "
            "camera_emotion.worker_mode to remote_worker and configure the "
            "emotion_worker_url on a supported Windows x64 or macOS Apple "
            "Silicon host."
        )
    return None
...
def _configure(context: PluginContext) -> None:
    ...
    if not deepface_home:
        deepface_home = str(
            _compatible_runtime_path(
                storage_root / "runtime" / "camera_emotion" / "worker" / "deepface_home",
                storage_root / "runtime" / "local_emotion_worker" / "deepface_home",
            )
        )
...
def _status(context: PluginContext) -> dict[str, Any]:
    ...
    if _config.get("worker_mode") == "local_worker":
        support_error = _local_worker_support_error()
        if support_error:
            return {
                **status,
                "status": "unsupported",
                ...
                "connected": False,
                "supported_modes": ["remote_worker"],
                "suggested_action": "Configure remote_worker and emotion_worker_url.",
                "last_message": support_error,
            }
...
def _start(context: PluginContext) -> Any:
    ...
    if _config.get("worker_mode") != "local_worker":
        return _status(context)
    if _local_worker_support_error():
        return _status(context)
...
def _runtime_error_message(error_info: dict[str, Any]) -> str:
    ...
    if error_class in {"model_download_failed", "model_file_missing", "model_file_unreadable"}:
        return (
            ...
            "Review THIRD_PARTY_NOTICES.md and explicitly provision the separately licensed model with "
            "'python release_tools/fetch_deepface_model_assets.py "
            "--accept-vgg-face-non-commercial-research-terms', or manually place the verified "
...
def _run_dependency_install(context: PluginContext) -> bool:
    ...
    for constraint_file in _DEPENDENCY_CONSTRAINT_FILES:
        command.extend(("--constraint", str(constraint_file)))
    command.extend(("--requirement", str(requirements_file)))
    output = ""
    return_code = 1
    message = ""
    ...
    if not requirements_file.exists():
        ...
        str(path) for path in _DEPENDENCY_CONSTRAINT_FILES if not path.is_file()
    ]:
        message = (
            "Dependency compatibility constraints are missing; refusing an "
            f"unconstrained repair: {', '.join(missing_constraints)}"
        )
    else:
        try:
...
def _run_model_asset_install() -> bool:
    ...
    try:
        ...
        if _asset_is_valid(asset_path):
            ...
            message = (
                "DeepFace model weights are not distributed or downloaded automatically. "
                "Review THIRD_PARTY_NOTICES.md, then run "
                "'python release_tools/fetch_deepface_model_assets.py "
...
def _compatible_runtime_path(canonical: Path, legacy: Path) -> Path:
    """Reuse an existing v2 cache, otherwise create new state canonically."""
...
def _ensure_local_model_weights() -> None:
    """Seed the runtime cache only from an operator-provisioned verified asset.

    Source releases intentionally contain no model weights. Runs before warmup
    so DeepFace never performs a silent download; a missing asset remains an
    actionable readiness error.
    """
    try:
        asset_path = _model_asset_path()
        if _asset_is_valid(asset_path):
...
def _asset_is_valid(path: Path) -> bool:
    try:
        return (
            path.is_file()
            and path.stat().st_size >= int(DEEPFACE_EMOTION_MODEL["min_bytes"])
            and _sha256(path) == DEEPFACE_EMOTION_MODEL_SHA256
        )
    except OSError:
        return False
...
def _bundled_model_asset_path() -> Path:
    return (
        Path(str(_config.get("model_assets_dir") or ""))
        / DEEPFACE_EMOTION_MODEL["name"]
    )
