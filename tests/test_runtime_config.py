...
from server_app.services.runtime_config import initialize_runtime_storage, resolve_runtime_paths
...
class RuntimeConfigTests(unittest.TestCase):
    def test_default_paths_stay_inside_project(self) -> None:
        ...
        self.assertEqual(paths.settings_dir, PROJECT_ROOT / "settings")
        self.assertEqual(paths.data_dir, PROJECT_ROOT / "saved_results")
        self.assertEqual(paths.saved_studies_dir, PROJECT_ROOT / "saved_studies")
    ...
    def test_external_data_dir_is_seeded_from_project_defaults(self) -> None:
        with tempfile.TemporaryDirectory() as base_dir, tempfile.TemporaryDirectory() as data_dir:
            base = Path(base_dir)
            (base / "settings").mkdir()
            (base / "settings" / "study_config.json").write_text('{"study_id": "Default"}', encoding="utf-8")
            (base / "settings" / "hardware_settings.json").write_text('{"lsl": {"enabled": false}}', encoding="utf-8")
            (base / "saved_studies").mkdir()
            (base / "saved_studies" / "Default.study-runner").write_text('{"study_id": "Default"}', encoding="utf-8")
            ...
            self.assertTrue((storage / "saved_studies" / "Default.study-runner").exists())
