...
>
> **Settings-container and editor pass (2026-08-03):** the settings shell is now the container, not a launcher. Certificate, app update, desktop shortcut, and the audit text render inside it as their own nav entries; `web/pages/audit.html` and the `/audit` route are gone. The hub lost the shortcut tile, the audit tile, and the update card. Per-plugin panels no longer show live status at all - configuration and live control are two surfaces, and duplicating the numbers at two refresh rates was the reason to split them. Machine and per-study settings now live in `web/scripts/settings/machine/` and `web/scripts/settings/study/`; the ~340-line hub block moved out of `admin-controller.js` into `settings/machine/machine-settings-panel.js`. In the card editor, the question text, instruction, note, and the toggle group are composed once in `openOverlay` around each card's own fields, in the order an author writes a question; the prompt field is no longer copied into nine card modules. Editor switches carry no explanatory paragraph and no filled row - the explanation is the hover title - and `.switch-row` keeps its old look for the participant page. "Open study" became **Preview** (`/?preview=1`): it renders the real study, sends no heartbeat so it never occupies the single-tablet slot, and drops every server write at one barrier. A **Play** button in the editor asks through the shared modal, starts the run, and lands on the dashboard; `startLoadedStudyRun` takes the button id and the destination, so both entry points share one path. All five remaining `confirm()` calls on the admin page are now `confirmWithModal`.
