"""Minimal extension SDK for building a new plugin (Phase 5j).
...
Every command works on a plugin folder *outside* `study_runner/extensions/`,
so trying things out here never touches the real, shipped extensions.
...
from study_runner.plugin_framework.extension_layout import (  # noqa: E402
...
TEMPLATES_DIR = Path(__file__).resolve().parent / "extension_templates"
...
def cmd_validate(plugin_dir: Path) -> int:
    ...
    `apps/server/__init__.py` calls for the real `extensions/` tree. Passing
...
def _fake_runtime_env(plugin_dir: Path) -> Iterator[None]:
    """Make a plugin folder outside `extensions/` findable by its own subprocess.
    ...
    environment variables (`extension_layout.py`). This function sets those
    ...
    os.environ[TEST_EXTRA_ROOT_PATH_ENV_VAR] = str(package_dir)
    os.environ[TEST_EXTRA_ROOT_PACKAGE_ENV_VAR] = package_name
    try:
        yield
    finally:
...
def cmd_new(category: str, key: str, out_dir: Path | None) -> int:
    ...
    print(
        ...
        f"Then: python tools/extension_sdk.py validate {destination}\n"
        f"      python tools/extension_sdk.py check-runtime {destination}"
    )
    return 0
...
def generate_schema() -> dict[str, Any]:
    ...
    return {
        ...
        "x-generated-by": "tools/extension_sdk.py generate_schema()",
        ...
        "x-note": (
            ...
            "`python tools/extension_sdk.py validate <dir>` for the real, "
        ...
        "properties": {
            ...
            "entry_point": {
                ...
                "description": (
                    "Vestigial as of api_version 4+ (Phase 3.1): validated if "
                    "present, never required, and not read by the process host."
                ),
            },
            "ui": {
                "type": "object",
            ...
            "capabilities": {
                "description": (
                    ...
                    "study_runner/extensions/README.md for the current, "
...
def cmd_schema(*, write: bool) -> int:
    ...
    if current != text:
        print(
            ...
            "run `python tools/extension_sdk.py schema --write`",
            file=sys.stderr,
        )
        return 1
